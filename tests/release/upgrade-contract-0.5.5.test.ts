// MECHANICAL SEAMLESS-UPGRADE PROOF: 0.5.5 -> this release.
//
// `tests/fixtures/backcompat/contract-0.5.5.json` is the CLI's JSON contract as
// captured from the REAL published 0.5.5 binary: every key path, its type, its
// value, and the process exit code, for each of the 16 steps of the daily loop
// over the fixed hermetic workspace.
//
// This release adds `rebind` / `unbind` and a `binding_released` event to the
// binding ledger. Adding an event type is breaking for OLDER readers (they reject
// what they do not know) — but nothing about it may change what an EXISTING
// project sees when it upgrades and carries on as before. That is the claim this
// test makes mechanical, with the same rules as the 0.4.3 contract test and the
// same crucial property:
//
//   THE DECLARED-CHANGES LIST IS EMPTY.
//
// Any key, type, shape, value, or exit code a 0.5.5 consumer could observe
// differing here fails the test. New keys are allowed; that is what an additive
// release does. The one-way part of the change is proved separately, in
...
const goldenPath = join(repoRoot, "tests/fixtures/backcompat/contract-0.5.5.json");
...
describe("this release is a strict superset of the 0.5.5 contract — no exceptions", () => {
  test("every step the 0.5.5 contract covers still exists", () => {
    const missing = golden.steps
      .map((step) => step.label)
      .filter((label) => stepOf(current, label) === undefined);
    expect(missing, "a command covered by the 0.5.5 contract disappeared").toEqual([]);
  });
  ...
  test("no type changed", () => {
    ...
    expect(retyped, "a type change breaks a consumer's parse — nothing is declared here").toEqual([]);
  });
  ...
  test("no scalar value changed", () => {
    ...
    expect(
      changed,
      "an existing project that upgrades and carries on as before must see NOTHING change"
    ).toEqual([]);
  });
});
